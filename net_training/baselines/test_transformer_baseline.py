...
class TransformerBaselineTester:
    ...
    def load_model(self, path):
        ...
        if os.path.exists(config_path):
            ...
            logger.info(f"Loaded architecture from config.json: d_model={d_model}, n_heads={n_heads}, encoder_layers={n_encoder_layers}, decoder_layers={n_decoder_layers}")
        else:
            d_model = 128
            n_heads = 4
            n_encoder_layers = 3
            ...
            dim_feedforward = 512
            dropout = 0.1
            logger.info("Config.json not found, using default transformer architecture")
        ...
        logger.info(f"Model type: TRANSFORMER (Baseline)")
    ...
    def evaluate(self, X_test, Y_test, logic_name=None, use_attacked=False, attack_type='gaussian', epsilon=1.5):
        ...
        with torch.no_grad():
            y_pred = self.model(X_test_tensor, x_condition=None)
    ...
    def test_adversarial(self, X_test, Y_test, n_samples=100, attack_type='gaussian', epsilon=1.5):
        ...
        with torch.no_grad():
            y_pred = self.model(X_attacked_tensor, x_condition=None)
    ...
    def visualize_predictions(self, X_test, Y_test, logic_name=None, n_samples=10, save_path='plots/transformer_baseline_predictions.png',
        ...
        title_text = f'Transformer Baseline Prediction Visualization ({n_samples} samples concatenated)'
        if use_attacked:
    ...
    def test_stl_satisfaction(self, X_test, Y_test, n_samples=100, attack_type='gaussian', epsilon=1.5, use_attacked=False, stl_formulas=None, directly_enforced=None, other_formulas=None):
        ...
        with torch.no_grad():
            y_pred = self.model(X_subset_tensor, x_condition=None)
        ...
        directly_enforced_final = None  # Will be set from first sample
        ...
        for formula_name, data in formula_results_all.items():
            ...
            if formula_name in directly_enforced_final:
        ...
        if results_other:
            logger.info("\n" + "=" * 80)
            logger.info("STL SATISFACTION/ROBUSTNESS RESULTS")
            logger.info("=" * 80)
    ...
    def test_stl_satisfaction_combined(self, X_test, Y_test, n_samples=100, attack_type='gaussian', epsilon=1.5):
        ...
        with torch.no_grad():
            y_pred_clean = self.model(X_clean_tensor, x_condition=None)
        y_pred_clean_np = y_pred_clean.cpu().numpy()
        
        # Get adversarial predictions
        X_attacked_tensor = torch.FloatTensor(X_attacked_np).to(self.device)
        with torch.no_grad():
            y_pred_attacked = self.model(X_attacked_tensor, x_condition=None)
        ...
        for i in range(n_samples):
            ...
            # Compact output: skip printing per-sample GT and formulas
    ...
    def _format_stl_readable(self, formula, feature_names):
        ...
        if isinstance(formula, Always):
            ...
            if formula.feature_idx is not None:
                # Map augmented feature indices to readable names
                feat_idx = formula.feature_idx
                base = len(feature_names)
                if feat_idx < base:
                    feat_name = feature_names[feat_idx]
                elif feat_idx < 2 * base:
                    feat_name = f"delta_{feature_names[feat_idx - base]}"
                elif feat_idx < 3 * base:
                    feat_name = f"abs_delta_{feature_names[feat_idx - 2 * base]}"
                else:
...
def main():
    parser = argparse.ArgumentParser(description='Test Transformer Baseline Model')
    parser.add_argument('--model', type=str, default='training_runs/baseline_transformer_20251229_120000/best_model.pth',
    ...
    # Allow switching dataset root via environment (e.g., pytorch_datasets_cali)
    ...
    tester = TransformerBaselineTester(model_path, device=device)
    ...
    tester.visualize_predictions(X_test_t, Y_test_t, logic_name=None, n_samples=args.n_vis, 
                                 save_path=f'plots/{model_name}_transformer_baseline_concatenated.png',
